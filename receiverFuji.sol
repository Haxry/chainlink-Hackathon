...
contract ReceiverFuji is CCIPReceiver {
    ...
    mapping(address=>string[]) userToData; // Store the last received text.
    ...
    function getLastPriceAndZkProof()
        external
        view
        returns (bytes32 messageId, string memory text)
    {
        uint256 length= userToData[_user].length;
        return (s_lastReceivedMessageId, userToData[_user][length-1]);
    }
