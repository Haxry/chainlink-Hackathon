...
contract PriceFeedAmznepolia {
    ...
    function getLatestPrice(address account) external view returns(uint256){
        uint256 length= addressToPriceArray[account].length;
        return addressToPriceArray[account][length-1];
    }

    function getLatestProof(address account) external view returns(bytes32){
        uint256 length= addressToProof[account].length;
        return addressToProof[account][length-1];
    }
